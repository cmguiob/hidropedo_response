...
### 1.1 Carga de polígono de Colombia para delimitar área de estudio
...
### 1.3 Descarga local y carga de SMAP
...
Los siguientes bloques cargan por separado los archivos de humedad del suelo SMAP para los periodos correspondientes a El Niño y La Niña, los apila como stacks y genera un raster de promedio y de coeficiente de variación para cada uno. Esta verificación es clave para inspeccionar la calidad espacial de los datos antes de evaluar la cobertura válida para el muestreo.
...
Este bloque identifica las celdas con datos suficientes (≥ 50% del pixel con mas valores onservados) en ambos periodos SMAP (Niño y Niña) y las restringe al área del polígono de Colombia. Esto garantiza que los puntos de muestreo se extraigan solo donde SMAP tiene cobertura adecuada en ambas condiciones climáticas.
...
```
Para garantizar que los puntos de muestreo se generen exclusivamente sobre las zonas con datos SMAP válidos (donde cobertura_final == TRUE), se convierte la máscara raster en un polígono vectorial. Este polígono se reproyecta a coordenadas métricas (EPSG:9377), se valida y luego se transforma en un objeto owin, compatible con {spatstat}. Esta ventana define con exactitud el área de muestreo para rSSI().


```{r prep_ventana_owin_colombia}

# 1. Cargar la máscara raster
cobertura_final <- terra::rast(here::here("Data", "SMAP_cobertura_valida_ambos_periodos.tif"))

# 2. Convertir zonas TRUE a polígonos (terra → sf)
poligono_valido <- cobertura_final |>
  terra::as.polygons(na.rm = TRUE, dissolve = TRUE) |>
  sf::st_as_sf()

# 3. Reproyectar a sistema métrico (EPSG:9377)
poligono_valido_proj <- sf::st_transform(poligono_valido, 9377)

# 4. Unificar geometrías y validar
poligono_valido_proj <- sf::st_make_valid(sf::st_union(poligono_valido_proj))

# 5. Convertir a owin (ahora sí funciona)
owin_valida <- spatstat.geom::as.owin(poligono_valido_proj)

plot(cobertura_final)

```

A partir de la ventana owin_valida, se genera un patrón de puntos aleatorios mediante el algoritmo rSSI() con una distancia mínima de 10 km entre puntos. El patrón se convierte en objeto sf, se transforma a EPSG:4326, y se valida visualmente sobre la máscara de cobertura usando ggplot2. Finalmente, se exporta como archivo .gpkg para su uso posterior.

```{r}
# 1. Ejecutar el muestreo inhibido
set.seed(123)
muestreo <- spatstat.random::rSSI(
  r = 10000,           # separación mínima: 10 km
  giveup = 10000,
  win = owin_valida,
  nsim = 1
)

cat("✔️ Puntos de muestreo generados:", muestreo$n, "\n")

# 2. Convertir los puntos a sf en EPSG:9377 (coincide con la ventana)
muestreo_sf_proj <- sf::st_as_sf(
  data.frame(x = muestreo$x, y = muestreo$y),
  coords = c("x", "y"),
  crs = 9377
)

# 3. Transformar a EPSG:4326 para compatibilidad con SMAP y GEE
muestreo_sf <- sf::st_transform(muestreo_sf_proj, 4326)

# 4. Preparar raster como sf para superponer en ggplot
cobertura_sf <- cobertura_final |>
...
ggplot() +
  geom_sf(data = cobertura_sf, fill = "gray90", color = NA) +
  geom_sf(data = muestreo_sf, color = "darkblue", size = 0.3, alpha = 0.8) +
  coord_sf(crs = 4326) +
  theme_minimal(base_size = 13) +
  labs(
    title = "Puntos de muestreo espacial sobre cobertura SMAP válida",
    subtitle = "Zonas válidas según cobertura_final (EPSG:4326)",
    caption = "Patrón rSSI con separación de 10 km"
  )
...
# 1. Cargar la máscara raster binaria de cobertura válida (TRUE/FALSE)
cobertura_final <- terra::rast(here::here("Data", "SMAP_cobertura_valida_ambos_periodos.tif"))

# 2. Filtrar solo los valores TRUE (los FALSE se vuelven NA)
cobertura_filtrada <- cobertura_final
cobertura_filtrada[!cobertura_filtrada == 1] <- NA

# 3. Convertir las zonas TRUE a polígonos (terra → sf)
poligono_valido <- cobertura_filtrada |>
  terra::as.polygons(dissolve = TRUE, na.rm = TRUE) |>
  sf::st_as_sf()

# 4. Verificación visual del multipolígono generado
plot(poligono_valido$geometry, col = "#cce5df", border = "gray40",
     main = "Zonas de cobertura SMAP válida (poligonizadas)")
```
...
# 5. Reproyectar polígonos a sistema métrico (EPSG:9377)
poligono_valido_proj <- sf::st_transform(poligono_valido, 9377)

# 6. Unificar geometrías y validar topología
poligono_valido_proj <- sf::st_make_valid(sf::st_union(poligono_valido_proj))

# 7. Convertir a objeto owin para uso con spatstat
owin_valida <- spatstat.geom::as.owin(poligono_valido_proj)

# 8. Ejecutar muestreo inhibido rSSI sobre la ventana de zonas válidas
set.seed(123)
muestreo <- spatstat.random::rSSI(
  r = 10000,
  giveup = 10000,
...
#Convertir a sf (EPSG:9377 → EPSG:4326)
muestreo_sf <- sf::st_as_sf(
  data.frame(x = muestreo$x, y = muestreo$y),
  coords = c("x", "y"),
  crs = 9377
) |>
...
# 10. Visualización con ggplot2 sobre el polígono de cobertura
library(ggplot2)

ggplot() +
  geom_sf(data = poligono_valido, fill = "gray90", color = NA) +
  geom_sf(data = muestreo_sf, color = "darkblue", size = 0.3, alpha = 0.8) +
  coord_sf(crs = 4326) +
  theme_minimal(base_size = 13) +
  labs(
    title = "Puntos de muestreo espacial sobre zonas SMAP válidas",
    subtitle = "Muestreo inhibido (r = 10 km) sobre cobertura_final",
    caption = "Fuente: SMAP NSIDC-0779"
  )

cat("\u2714\ufe0f Puntos de muestreo generados:", muestreo$n, "\n")


# 12. Exportar como archivo .qs a Google Drive
qs::qsave(
  muestreo_sf,
  file = "puntos_muestreo_rSSI.qs"
)

googledrive::drive_upload(
  media = "puntos_muestreo_rSSI.qs",
  path = as_id("1PhfwOQOuHXmEIKE20i2-nqZaRrRfW0V9"),
  name = "puntos_muestreo_rSSI.qs",
  overwrite = TRUE
)

cat("\u2714\ufe0f Exportado como .qs a Google Drive\n")


# 11. Exportar como GeoPackage
sf::st_write(
  muestreo_sf,
  dsn = here::here("Data", "puntos_muestreo_rSSI.gpkg"),
  delete_dsn = TRUE,
  quiet = TRUE
)

cat("\u2714\ufe0f Exportado como 'Data/puntos_muestreo_rSSI.gpkg' (EPSG:4326)\n")
